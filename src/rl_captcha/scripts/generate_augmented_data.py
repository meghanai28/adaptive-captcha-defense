"""Pre-generate adversarially augmented bot sessions.

Applies the exact same HumanProfiler + 3-tier augmentation pipeline used
by the classifier (Section 3.5.3).  For every bot session in data/bot/,
generates ``n_copies_per_level × 3`` humanized copies and saves them as
individual JSON files in data/bot_augmented/.

Usage:
    python -m rl_captcha.scripts.generate_augmented_data \
        --data-dir data/ \
        --n-copies 2 \
        --seed 42
"""
...
def main():
    ...
    sessions = load_from_directory(str(data_dir))
    ...
    # Generate augmented sessions (identical to classifier pipeline)
